...
async(foodData, thunkAPI) =>{
    try {
      const token = thunkAPI.getState().authoslice.user.token
...
//get  food action
export const getFoodAction = createAsyncThunk('food/single',
async(foodId, thunkAPI) =>{
    try {
...
export const foodSlice = createSlice({
    ...
    .addCase(getFoodAction.pending, (state) => {
      state.isLoading = true
  })
  .addCase(getFoodAction.fulfilled, (state, action) => {
      state.isLoading = false
      state.isFound = true
      state.foodItem = state.foods.filter((food) => food._id === action.payload.id)
  })
  .addCase(getFoodAction.rejected, (state, action) => {
      state.isLoading = false
      state.isError = true
      state.message = action.payload
  })
